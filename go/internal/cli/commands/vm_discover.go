...
func (f *simulatorFilter) learn(ctx context.Context, name string, port int) {
	...
	for {
		if hostname := askVMHostname(ctx, port); hostname != "" {
...
func askVMHostname(ctx context.Context, port int) string {
	...
	_, resp, err := getAgentVersionAtAddress(actx, hostAddrFor(defaultAgentPort))
	if err != nil || resp == nil {
		return ""
	}
